...
public class Node {
	...
	public boolean equals(Object node) {
		if (node instanceof Node) {
			return type == ((Node) node).type && value.equals(((Node) node).value) && dependentNodes.containsAll(((Node) node).dependentNodes) && dependentNodes.size() == ((Node) node).dependentNodes.size();
		}
